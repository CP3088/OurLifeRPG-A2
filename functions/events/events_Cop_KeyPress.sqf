...
fnc_KeyPress_F5 = {
	...
	if(INV_shortcuts)then{INV_shortcuts=false;titletext["OL keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["OL keys on", "PLAIN DOWN"];[] execVM "actions\fnc_West_Actions.sqf"};
};
