...
fnc_KeyPress_F5 = {
	...
	if(INV_shortcuts)then{INV_shortcuts=false;titletext["OL keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["OL keys on", "PLAIN DOWN"];[] execVM "actions\fnc_GUER_Actions.sqf"};
};
	...
	if((vehicle player == player) and call INV_isArmed) then
		...
		_men spawn
			...
			if(count _this > 0) then
				...
				format['%1 setVariable ["Cuffed", false,true]', _civ] call broadcast;
